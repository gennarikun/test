...
public class PurchaseHistoryInfoDAO {
	...
	public List<PurchaseHistoryInfoDTO> getPurchaseHistoryList(String loginId) {
		...
		String sql = "SELECT " + "phi.id AS id, " // ID
				...
				+ "FROM purchase_history_info AS phi " + "LEFT JOIN product_info AS pi "
				+ "ON phi.product_id = pi.product_id " // それぞれの商品IDをリンク
				+ "LEFT JOIN destination_info AS di " + "ON phi.destination_id = di.id " // それぞれの宛先情報IDをリンク
	...
	public int regist(String loginId, int productId, int productCount, int destinationId, int price) {
		DBConnector dbConnector = new DBConnector();
		Connection connection = dbConnector.getConnection();
		String sql = "INSERT INTO purchase_history_info(" + "user_id, " // ユーザーID
				+ "product_id, " // 商品ID
				+ "product_count, " // 購入個数
				+ "price, " // 金額(合計?)
				+ "destination_id, " // 宛先ID
